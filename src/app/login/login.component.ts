      ...
      this.authService.loginUser(req).subscribe(
        (res: any) => {
          ...
          sessionStorage.setItem(
            'userid',
            res.body.data.responseObj1.response.responseData.user_id
          );
          if (res.body.data.responseObj1.response.responseData.office_id) {
            sessionStorage.setItem(
              'officeId',
              res.body.data.responseObj1.response.responseData.office_id || '88'
            );
          } else {
          ...
          if (this.loginData.length <= 0) {
            ...
            this.checkAuthorisation(
              res.body.data.responseObj1.response.responseData.id
            );
          }
        },
        (err) => {
